...
public class Block implements ISupportMemento {
    ...
    protected Block(Workspace workspace, Long id, String genusName, String label, boolean linkToStubs) {
        ...
        final BlockGenus genus = this.env.getGenusWithName(genusName);
        if (genus == null) {
    ...
    private void expandSocketGroup(String group) {
        List<BlockConnector> expandSockets = getExpandGroup(this.expandGroups, group);
        assert expandSockets != null;

        // Search for the socket to insert after.
        int index = this.sockets.size() - 1;
        String label = expandSockets.get(expandSockets.size() - 1).getLabel();
        for (; index >= 0; index--) {
            BlockConnector conn = this.sockets.get(index);
    ...
    private void shrinkSocketGroup(BlockConnector socket) {
        ...
        for (; index >= 0; index--) {
            BlockConnector con = this.sockets.get(index);
    ...
    public void blockConnected(BlockConnector connectedSocket, Long connectedBlockID) {
        ...
        if (hasStubs()) {
            BlockStub.parentConnectorsChanged(this.workspace, getBlockID());
        }
    }
    ...
    public void removeSocket(int index) {
        removeSocket(this.sockets.get(index));
    }
	...
	public void loadState(Object memento) {
        if (memento instanceof BlockState) {
            ...
            if (state.plug == null) {
                ...
                this.plug = BlockConnector.instantiateFromState(this.workspace, state.plug);
            }

            if (state.before == null) {
                this.before = null;
            } else {
                this.before = BlockConnector.instantiateFromState(this.workspace, state.before);
            }

            if (state.after == null) {
                this.after = null;
            } else {
                this.after = BlockConnector.instantiateFromState(this.workspace, state.after);
            }
